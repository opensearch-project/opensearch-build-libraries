        ...
        if (repoType.equals("apt")) {
            ...
            string(credentialsId: 'jenkins-rpm-signing-key-id', variable: 'RPM_SIGNING_KEY_ID')]) {
                ...
                sh """#!/bin/bash
                     ...
                     aptly repo create -comment="Opensearch Repository" -distribution=stable -component=main ${jobname}
                     ...
                     aptly publish snapshot -label="Opensearch" -origin="opensearch.org" -batch=true -passphrase-file=passphrase ${jobname}-${repoVersion}
                     echo "------------------------------------------------------------------------"
                     echo "Clean up gpg"
