 ...
 *  @param args.excludeTasksFilePath <optional> - Path to a file containing gradle tasks to exclude (one per line), e.g. gradle-check-excludes.txt
 **/
...
void call(Map args = [:]) {
    ...
    def exclude_tasks_file_path = args.excludeTasksFilePath ?: ''
    ...
    if (exclude_tasks_file_path && fileExists(exclude_tasks_file_path)) {
        def excludeLines = readFile(exclude_tasks_file_path).trim().split('\n')
        exclude_tasks_param = excludeLines.collect { "-x ${it.trim()}" }.join(' ')
        println("Excluding tasks: ${exclude_tasks_param}")
    }
